...
class OrderService {
  ...
  async createOrderIntent(price: number, buyerId: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.post('/create-payment-intent', {
      price,
      buyerId
    });
    return response;
  ...
  async createOrder(body: IOrderDocument): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.post('/create', body);
    return response;
  }
  ...
  async deliverOrder(orderId: string, body: IOrderMessage): Promise<AxiosResponse> {
  ...
  async getNotifications(userTo: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.get(`/notifications/${userTo}`);
    return response;
  }

  async markNotificationAsRead(notificationId: string): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.put(`/notifications/${notificationId}`);
    return response;
  }
}
